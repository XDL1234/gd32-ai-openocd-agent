...
### Step 0: 会话恢复检查（五问重启测试）

首先检查是否存在四文件，如果存在则执行会话恢复检查：
...
1. **读取 `docs/项目规划清单.md`** → 回答：
   - 当前处于哪个阶段（RESEARCH/INNOVATE/PLAN/EXECUTE/REVIEW）？
   - 哪些功能模块已完成？哪些正在进行？
   - 下一步应该从哪里继续？

2. **读取 `docs/编辑清单.md`** → 回答：
   - 最后一次代码修改是什么？
   - Git 提交状态如何（hash、是否已 push）？
   - 是否有回档记录？

3. **读取 `hardware/硬件资源表.md`** → 回答：
   - 芯片型号和开发框架已确认吗？
   - 当前引脚分配情况如何？是否有冲突？

4. **读取 `docs/研究发现.md`** → 回答：
   - 之前的搜索发现了什么关键技术或方案？
   - 有哪些待验证的线索或已排除的方案？

5. **确认当前阶段** → 基于前四个文件的内容，回答：
   - 我现在应该从哪里继续？
   - 是否需要回退到前一个阶段？
